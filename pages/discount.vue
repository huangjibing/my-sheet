...
const searchKey = ref('Storewide: 15% Off');

const baseStore = useBaseStore();
const productList = ref(
  searchKey.value ? baseStore.getProducParentTypetList(searchKey.value) : []
);
...
const handleInit = (txt?: string) => {
  ...
  if (txt) {
    searchKey.value = txt;
    productList.value = baseStore.getProducParentTypetList(searchKey.value);
  ...
  if (productList.value.length) {
    curKey.value = baseStore.classifyDiscountList[0];
  } else {
    curKey.value = "";
  }
};
...
const sortClick = () => {
  ...
  productList.value = searchKey.value ? baseStore.getProducParentTypetList(searchKey.value) : [];
}
</script>
...
<template>
  ...
  <div class="search-wrapper">
    <!-- <Search :searchTxt="searchKey" @onSearch="handleInit" /> -->
    <!-- <div class="tab-layout" v-if="tabList.length">
      <TabBar
        :tabList="tabList"
        :activeKey="curKey"
        :isTranslate="false"
        @changeKey="handleChangeKey"
      />
    </div>
    <div class="search-products-list" v-if="productCurList.length">
      <ProductItem
    ...
    <div class="goods">
      ...
      <div class="products-list">
        ...
        <div class="not-data" v-if="productCurList.length === 0">
          <img src="@/assets/imgs/products_empty.png" alt="">
          <div>No products found</div>
        </div>
      </div>
    </div>
